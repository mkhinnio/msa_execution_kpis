...
for i in range(0,len(partitioned_list)-1):  #len(partitioned_list)
    ...
    df_fc_harmonized_costs_to_use_export_grouped=df_fc_harmonized_costs_to_use_export.groupby(["opportunity_number","unit_serial_number","billing_type","frequency"]).aggregate({"cost_occurrence_product":"sum","occurrence":"mean"}).reset_index()
    ...
    df_fc_outputs_grouped=df_fc_harmonized_costs_to_use_export_grouped_output.groupby(["opportunity_number","unit serial number","contract number",
                                                                                    "billing_type","frequency"]).aggregate({"cost_tc":"sum","occurrence":"sum"})
...
df_fc_harmonized_costs_to_use_export_grouped_output.to_excel("output_costs_export_sample_all_years_frequency_mapping_" + date_today + ".xlsx")

df_fc_outputs_grouped.to_excel("grouped_sample_sr_mapping_frequency_mapping_" + date_today + ".xlsx")
