...
def return_srs_gl(conn, list_srs):
    
    
    list_srs=','.join("'"+str(x)+"'" for x in list_srs)
    list_srs="("+list_srs+")"
    query_sr_numbers = f"SELECT * FROM pgsdwh.sot_gps_dp.dwh_dm_idl_trx_actual_margin_se_v WHERE service_request_number IN {list_srs} " 
    sr_overview = pd.read_sql(query_sr_numbers, conn) 
    return sr_overview


def return_srs_events(conn, list_srs):
    
    
    list_srs=','.join("'"+str(x)+"'" for x in list_srs)
    list_srs="("+list_srs+")"
    query_sr_numbers = f"SELECT * FROM pgsdwh.myplant.maintenance_events_dmp_events WHERE sr_number IN {list_srs} " 
    sr_overview = pd.read_sql(query_sr_numbers, conn) 
    return sr_overview
...
def import_corner_point_assets(conn):
    '''
    highlevel data
    '''
    query = """
            SELECT * 
            FROM myplant.engine_forecast_engine_counter_corner_point where counter_type = 'OPH' AND source = 'FORECAST' AND asset_id in (136729, 136726, 136632, 136723, 136717)---'HISTORY' 
            """
    engine_forecast_engine_counter_corner_point = pd.read_sql(query, conn) 
    return engine_forecast_engine_counter_corner_point
...
def import_corner_point(conn):
    ...
    query = """
            SELECT * 
            FROM myplant.engine_forecast_engine_counter_corner_point where counter_type = 'OPH' AND source = 'FORECAST'---'HISTORY' 
            """
    engine_forecast_engine_counter_corner_point = pd.read_sql(query, conn) 
    return engine_forecast_engine_counter_corner_point
